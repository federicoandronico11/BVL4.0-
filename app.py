...
# ─── CARICAMENTO STATO (prima del CSS per usare theme) ──────────────────────
if "state" not in st.session_state:
    st.session_state.state = load_state()

state = st.session_state.state

# ─── CSS GLOBALE (tema da state, senza crash) ───────────────────────────────
theme = state.get("theme", "dazn_dark")
inject_css(theme)
...
with st.sidebar:
    ...
    idx_attuale = ordine.index(fase_corrente) if fase_corrente in ordine else 0
    ...
    # Profili Giocatori (carte FC26) — sempre accessibile
    if st.button("👤 Profili Giocatori", use_container_width=True,
                 type="primary" if fase_corrente == "profili" else "secondary", key="nav_profili"):
        state["fase"] = "profili"
        save_state(state)
        st.rerun()
    ...
    # Tema (accesso rapido; personalizzazione completa in Setup → Personalizzazione tema)
    st.markdown("**🎨 Tema**")
    theme_opts = ["dazn_dark", "dazn_red", "dark_blue", "dark_green"]
    theme_labels = {"dazn_dark": "Scuro DAZN", "dazn_red": "Rosso DAZN", "dark_blue": "Blu scuro", "dark_green": "Verde scuro"}
    idx = theme_opts.index(state.get("theme", "dazn_dark")) if state.get("theme") in theme_opts else 0
    ...
    if new_theme != state.get("theme"):
        ...
        save_state(state)
        st.rerun()
    st.caption("Altre opzioni in Setup → Personalizzazione tema")
...
if fase == "setup":
    ...
    st.caption("Carte in stile FC26 Ultimate Team: Overall e tier (Bronzo → GOAT) calcolati da tornei e statistiche.")
    st.divider()
