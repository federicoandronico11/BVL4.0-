...
def empty_state():
    return {
        ...
        "torneo": {
            "nome": "",
            "sede": "",                     # opzionale: luogo/sede
            "tipo_tabellone": "Gironi + Playoff",
            "formato_set": "Set Unico",
            "punteggio_max": 21,
            "punteggio_tie_break": 15,      # terzo set in Best of 3
            "data": str(datetime.today().date()),
            "num_gironi": 2,
            "squadre_per_girone": 0,
            "passano_per_girone": 2,
            "criterio_passaggio": "classifica",
            "girone_unico": False,
        },
...
def compute_attributes(atleta):
    """
    Restituisce i 6 attributi stile FIFA/FC per una carta:
    ATT, DEF, ALZ, RIC, MUR, BAT (40-99).
    Crescono in proporzione ai risultati su tutti i tornei.
    """
    s = atleta.get("stats", {})
    tornei = s.get("tornei", 0)
    vittorie = s.get("vittorie", 0)
    sconfitte = s.get("sconfitte", 0)
    set_vinti = s.get("set_vinti", 0)
    set_persi = s.get("set_persi", 0)
    punti_fatti = s.get("punti_fatti", 0)
    punti_subiti = s.get("punti_subiti", 0)

    overall = compute_overall(atleta)

    tot_set = set_vinti + set_persi
    ratio_set = (set_vinti - set_persi) / (tot_set or 1)  # da -1 a 1
    tot_pts = punti_fatti + punti_subiti
    ratio_pts = (punti_fatti - punti_subiti) / (tot_pts or 1)  # da -1 a 1
    win_rate = vittorie / (tornei or 1) if tornei > 0 else 0.0
    tornei_norm = min(tornei, 20) / 20.0

    def _attr(delta: float) -> int:
        return max(40, min(99, int(round(overall + delta))))

    return {
        "ATT": _attr(8 * ratio_pts + 4 * win_rate),          # attacco: punti fatti + vittorie
        "DEF": _attr(6 * ratio_set + 3 * win_rate),          # difesa: set a favore + solidità generale
        "ALZ": _attr(4 * win_rate + 2 * tornei_norm),        # alzata: esperienza e continuità
        "RIC": _attr(4 * ratio_set + 4 * win_rate),          # ricezione: stabilità nei set e vittorie
        "MUR": _attr(6 * ratio_set + 1 * tornei_norm),       # muro: differenza set e presenza
        "BAT": _attr(5 * win_rate + 3 * ratio_pts),          # battuta: impatto su punti e vittorie
    }
