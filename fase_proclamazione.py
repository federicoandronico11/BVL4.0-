...
def render_schede_carriera(state):
    ...
    if not state.get("atleti"):
        st.info("Aggiungi atleti in Setup per vedere le carte. I nuovi giocatori partono con Overall 40 (Bronzo Raro).")
        return
    
    # Tutti gli atleti (anche 0 tornei) per mostrare carte Overall 40
    nomi = [a["nome"] for a in state["atleti"]]
    selezionato = st.selectbox("🔍 Seleziona Atleta", nomi, key="career_select")
    
    atleta = next((a for a in state["atleti"] if a["nome"] == selezionato), None)
    if not atleta:
        return
    ...
    if s["storico_posizioni"]:
        ...
        for torneo_nome, pos in storico:
            icon = medals.get(pos, "")
            pos_label = f"{pos}º posto" if icon else f"{pos}° posto"
            st.markdown(f"• {icon} **{torneo_nome}** — {pos_label}")
